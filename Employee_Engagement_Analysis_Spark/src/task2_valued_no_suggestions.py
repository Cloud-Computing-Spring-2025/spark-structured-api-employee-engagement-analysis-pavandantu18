...
def identify_valued_no_suggestions(df):
    ...
    valued_df = df.filter(col("SatisfactionRating") >= 4)
    
    # 2. Among these, filter those with ProvidedSuggestions == False.
    valued_no_suggestions_df = valued_df.filter(col("ProvidedSuggestions") == False)
    
    # 3. Calculate the number and proportion of these employees.
    number = valued_no_suggestions_df.count()
    total_employees = df.count()
    proportion = round((number / total_employees) * 100, 2) if total_employees > 0 else 0.0
...
def main():
    ...
    output_file = "/workspaces/spark-structured-api-employee-engagement-analysis-pavandantu18/Employee_Engagement_Analysis_Spark/outputs/task2/valued_no_suggestions.txt"
